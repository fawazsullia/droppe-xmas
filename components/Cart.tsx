...
const Cart = ({approvedProds, setviewPopup, settotal, cart, count}: ICartProps) : JSX.Element  => {


  //handle when > button is pressed. redirects to page /confirm
  function handleConfirm(){
    ...
    cart.forEach((c)=> {  cartCount = cartCount + c.products.length})
    if(cartCount === count){
      setviewPopup(true)
    }
    else {
  ...
  return <div className={styles.container}>
    ...
    {approvedProds.map((prod) => {
      return <div key={prod.product.title} className={styles.product}>
