...
function Confirm({ approvedProds, setviewPopup, total, cart, rejectedProds }: IConfirmProps) {
  ...
  function getAppProds(kidId: number) {
    let app1: IApprovedProd[] = approvedProds.filter((p) =>p.kids.includes(kidId));

    return app1;
  }

  //get products that are not approved
  ...
  //I need to display data related to each child, both approved and disapproved
...
Promise.all(promises)
      .then((responses) => { router.push('/confirmation')  })
      .catch((e) => {
...
//   console.log(finalCarts);
  ...
  return (
    <div className={styles.container}>
        ...
        <p onClick={handleClosePopup} className={styles.clsBtn}>✖</p>
                ...
                finalCarts.map((item)=> {
                    return <div key={item.id} className={styles.card}>
                        ...
                        <ul className={styles.approvedDiv}>
                            {item.approvedProducts.map((p)=> <li key={`${p.product.id}+${p.kids[0]}`}>{p.product.title}</li>)}
                            </ul>
                            <br />
                        <ul className={styles.disapprovedDiv}>
                            {item.rejectedProducts.map((p)=> <li key={`${p.product.id}+${p.kids[0]}`}>{p.product.title}</li>)}</ul>
                            </div>
                    </div>
                })
            }
